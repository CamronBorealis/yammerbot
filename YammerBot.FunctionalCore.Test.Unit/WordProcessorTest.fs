...
[<TestFixture>]
type ``Given a list of definitions`` ()=
    [<Test>] member test.
     ``IsNoun returns true if there is a noun definition`` ()=
        WordProcessor.IsNoun TestData.nounDefs |> should be True

    [<Test>] member test.
     ``IsNoun returns false if there is no noun definition`` ()=
        WordProcessor.IsNoun TestData.nonNounDefs |> should be False

    [<Test>] member test.
     ``IsNoun returns false for an unknown word`` ()=
...
type ``Given a phrase and a list of definitions`` ()=
    ...
    [<Test>] member test.
     ``Sexify should not alter phrases without nouns`` ()=
        WordProcessor.Sexify "this is a " definitions |> should equal "this is a"
